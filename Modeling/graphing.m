...
switch plotType
    ...
    case {2, 3}
        ...
        for i=1:length(plotvec)-1
            if type ==1
                plot(plotdata(plotvec(i):plotvec(i+1),1),plotdata(plotvec(i):plotvec(i+1),2),plotcolor(1), 'LineWidth', 3.5)
                if plotType==3
                    plot(plotdata(plotvec(i):plotvec(i+1),1),ones(plotvec(i+1)-plotvec(i)+1,1)*data.yoffset(i),plotcolor(3), 'LineWidth', 3.5)
                end
                type=2;
                
            elseif type ==2
                plot(plotdata(plotvec(i):plotvec(i+1),1),plotdata(plotvec(i):plotvec(i+1),2),plotcolor(2), 'LineWidth', 3.5)
                if plotType==3
                    plot(plotdata(plotvec(i):plotvec(i+1),1),ones(plotvec(i+1)-plotvec(i)+1,1)*data.yoffset(i),plotcolor(3), 'LineWidth', 3.5)
                end
                type=1;
        ...
        if plotType==2
            lgd=legend('Stance', 'Flight','Location','southoutside','Orientation','horizontal');
            lgd.NumColumns = 2;
        elseif plotType==3
