function xdot=stance_motor_ode_poly_noise(t,x,IC,robot,simp, motor,func_param,mvar,V,type)
% Inputs:
%
...
for i=1:length(t)
    ...
    if t(i)<func_param.t_uncert
        volts(i)=0;

    else

        for j=1:length(V)
            %if time is less than delay then would still be 0.
            %otherwise evaluation time is time-delay
            volts(i)=volts(i)+V(j)*(t(i)-func_param.t_uncert)^(j-1);
        end
    end
end
