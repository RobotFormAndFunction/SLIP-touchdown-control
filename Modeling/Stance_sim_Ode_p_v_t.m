function [data]=Stance_sim_Ode_p_v_t(robot_param, IC,sim_param, iter, motor_parameter, func, func_param)
% Stance_sim_ode_p_v_t will run a complete leg stance cycle starting from
% stance where leg position can vary as a function of time *Note all motor
% functionality has been removed but varaible is still needed
%
% Inputs:
%   Robot_Param class
%
...
for i=1:2*iter
    ...
    switch modelType
        ...
        case 1  %Run stance model
            ...
            elseif isequal(flightmodel,@flight_p_v_t)
                p=rec_states(end,:);
                fIC=[p(1),p(2),p(4),p(5), -IC(3)];
        ...
        case 2
            ...
            % phi=length(yout(end,5);
            % if t<=func_param.apex_t
            %     td=func_param.td_start;
            % else
            %     td=polyval(func_param.p_v_t,t);
            % end
            phi=polyval(func_param.p_v_t,tt(end));
            ...
            for kpl=1:length(tt)
                if tt(kpl)<=func_param.apex_t
                    yout(kpl,5)=func_param.td_start;
                else
                    yout(kpl,5)=polyval(func_param.p_v_t,tt(kpl));
                end
            end
...
if x(4)<0
    ...
    value = x(3)-robot.l0*sin(td) - func_param.yoffset;
